...
echo -e "[ℹ]  Creating CNAME record for the server ${PURPLE}$SERVER${NC} and service ${BLUE}$SERVICE${NC} using ${CYAN}$EXPOSE${NC}."

#------------

# Get external hostname of Ingress Controller
INGRESS=`kubectl get --namespace domino ingress $SERVER-domino -o jsonpath='{.status.loadBalancer.ingress[0].hostname}'`
echo "[ℹ]  Ingress Controller external hostname: $INGRESS"

# Get external hostname of Load Balancer
LB=`kubectl get --namespace domino service $SERVER-domino-ext -o jsonpath='{.status.loadBalancer.ingress[0].hostname}'`
...
# Selet the value of the CNAME record (to which hostname it should point)
...
echo -e "[ℹ]  Desired state: ${YELLOW}(CNAME) $FULL_HOSTNAME ---> $TARGET_HOSTNAME${NC}"

#------------

echo -e "\n[ℹ]  Updating DNS CNAME record for $FULL_HOSTNAME in domain $DOMAIN"
